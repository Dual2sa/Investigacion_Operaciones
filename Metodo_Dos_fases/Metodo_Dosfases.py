...
class ProgramaPL:
    ...
    def definir_restriccion(self, variables):
        ...
        coeficientes = [self.obtener_entrada_numerica(f"Digite el coeficiente de {var.name} de la {nombre_restriccion} :") * var for var in variables[1:]]
        rhs = self.obtener_entrada_numerica('Digite el solucion de la restricción:')
    ...
    def ejecutar(self):
        while True:
            self.limpiar_consola()
            print("Programa desarrollado por David Santiago Aldana Pita - Ivan Santiago Bejarano Ramos")
